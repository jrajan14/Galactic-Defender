...
class Player {
    ...
    draw() {
        ...
        // Engine glow
        const engineGlow = ctx.createRadialGradient(this.x, this.y + this.height / 2 + 10, 0, this.x, this.y + this.height / 2 + 10, 20);
        ...
        if (this.shieldActive) {
            ...
            ctx.stroke();
            // Shield glow
            const shieldGlow = ctx.createRadialGradient(this.x, this.y, this.shieldRadius - 10, this.x, this.y, this.shieldRadius);
    ...
    shoot() {
        ...
        // Shooting particles
        for (let i = 0; i < 10; i++) {
            particles.push(new Particle(this.x + (Math.random() * 20 - 10), this.y + this.height / 2, (Math.random() - 0.5) * 2, Math.random() * 2, '#00ffff', 1, 1));
        }
    }
    activateShield() {
...
class Bullet {
    constructor(x, y, dx, dy, color, radius = 5, damage = 1) {
        this.x = x;
        this.y = y;
        this.dx = dx;
        this.dy = dy;
...
class Enemy {
    ...
    draw() {
        ...
        // Engine glow
        const engineGlow = ctx.createRadialGradient(this.x, this.y + this.height / 2 + 5, 0, this.x, this.y + this.height / 2 + 5, 15);
        engineGlow.addColorStop(0, 'rgba(255, 100, 0, 0.8)');
        engineGlow.addColorStop(1, 'rgba(255, 100, 0, 0)');
        ctx.fillStyle = engineGlow;
    ...
    shoot() {
        ...
        // Shooting particles
        for (let i = 0; i < 5; i++) {
            particles.push(new Particle(this.x + (Math.random() * 10 - 5), this.y + this.height / 2 + 10, (Math.random() - 0.5) * 1, Math.random() * 2, '#ff5555', 1, 1));
        }
    }
}
class PowerUp {
    constructor() {
        ...
        const types = ['health', 'double', 'laser', 'shield'];
...
class Explosion {
    constructor(x, y, color, maxRadius = 30, life = 0.5) {
        this.x = x;
        this.y = y;
        this.radius = 0;
...
function checkCollisions() {
    ...
    powerUps.forEach((powerUp, pIndex) => {
        ...
        if (distance < minDistance) {
            ...
            for (let i = 0; i < 20; i++) {
                particles.push(new Particle(powerUp.x, powerUp.y, (Math.random() - 0.5) * 5, (Math.random() - 0.5) * 5, powerUp.color, Math.random() * 3 + 1, Math.random() * 0.5 + 0.5));
            }
        ...
        if (bullet.dy < 0) { // Only player bullets move upward
            enemies.forEach((enemy, eIndex) => {
                ...
                if (distance < minDistance) {
                    ...
                    for (let i = 0; i < 5; i++) {
                        particles.push(new Particle(bullet.x, bullet.y, (Math.random() - 0.5) * 3, (Math.random() - 0.5) * 3, enemy.color, Math.random() * 2 + 1, Math.random() * 0.5 + 0.2));
                    }
                    // Remove bullet
                    bullets.splice(bIndex, 1);
                    ...
                    if (enemy.health <= 0) {
                        ...
                        for (let i = 0; i < 20; i++) {
                            particles.push(new Particle(enemy.x, enemy.y, (Math.random() - 0.5) * 5, (Math.random() - 0.5) * 5, enemy.color, Math.random() * 3 + 1, Math.random() * 0.5 + 0.5));
                        }
                        enemies.splice(eIndex, 1);
                        // Check for level up
                        checkLevelUp();
...
function checkLevelUp() {
    ...
    if (level - 1 < levelThresholds.length && score >= levelThresholds[level - 1]) {
        ...
        for (let i = 0; i < 50; i++) {
            particles.push(new Particle(GAME_WIDTH / 2, GAME_HEIGHT / 2, (Math.random() - 0.5) * 10, (Math.random() - 0.5) * 10, `hsl(${Math.random() * 360}, 100%, 50%)`, Math.random() * 4 + 2, Math.random() * 1 + 0.5));
        }
    }
}
function createExplosion(x, y, color, maxRadius = 30, life = 0.5) {
...
function gameOver() {
    ...
    for (let i = 0; i < 100; i++) {
        particles.push(new Particle(player.x, player.y, (Math.random() - 0.5) * 10, (Math.random() - 0.5) * 10, `hsl(${Math.random() * 360}, 100%, 50%)`, Math.random() * 5 + 2, Math.random() * 1 + 0.5));
    }
}
